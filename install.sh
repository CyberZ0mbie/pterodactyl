...
copyright(){
    output "Pterodactyl Installation & Upgrade script v16.3.1."
...
get_distribution() {
    ...
    if [ "$lsb_dist" =  "ubuntu" ]; then
        if [ "$dist_version" != "18.04" ] && [ "$dist_version" != "16.04" ] && [ "$dist_version" != "14.04" ]; then
        ...
        if [ "$dist_version" != "28" ] && [ "$dist_version" != "27" ]; then
...
server_options() {
    output "Please select what you would like to install:\n[1] Install the panel.\n[2] Install the daemon.\n[3] Install the panel and daemon.\n[4] Upgrade 0.7.x panel to 0.7.10.\n[5] Upgrade 0.6.x daemon to 0.6.7.\n[6] Emergency MariaDB root password reset."
    read choice
    case $choice in
        1 ) installoption=1
...
EOF
        ...
        elif  [ "$lsb_dist" =  "fedora" ] && [ "$dist_version" = "27" ]; then
...
gpgcheck=1
EOF
            yum -y install https://download1.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://download1.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm
            yum -y install https://rpms.remirepo.net/fedora/remi-release-28.rpm
...
EOF
            ...
            yum -y install https://download1.rpmfusion.org/free/fedora/rpmfusion-free-release-$(rpm -E %fedora).noarch.rpm https://download1.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-$(rpm -E %fedora).noarch.rpm
            yum -y install https://rpms.remirepo.net/fedora/remi-release-27.rpm
...
EOF
        ...
        elif  [ "$lsb_dist" =  "rhel" ] && [ "$dist_version" = "7" ]; then
...
install_dependencies(){
    ...
    if  [ "$lsb_dist" =  "ubuntu" ] ||  [ "$lsb_dist" =  "debian" ]; then
        ...
        if [ "$webserver" = "1" ]; then
            yum -y install php php-common php-fpm php-cli php-json php-mysqlnd php-mcrypt php-gd php-mbstring php-pdo php-zip php-bcmath php-dom php-opcache mariadb-server redis cronie nginx git policycoreutils-python-utils libsemanage-devel unzip
        elif [ "$webserver" = "2" ]; then
            yum -y install php php-common php-fpm php-cli php-json php-mysqlnd php-mcrypt php-gd php-mbstring php-pdo php-zip php-bcmath php-dom php-opcache mariadb-server redis cronie httpd git policycoreutils-python-utils libsemanage-devel mod_ssl unzip
        fi
    fi
    ...
    if [ "$lsb_dist" =  "fedora" ] && [ "$dist_version" = "27" ]; then
...
upgrade_pterodactyl(){
    cd /var/www/pterodactyl
    php artisan down
    curl -L https://github.com/pterodactyl/panel/releases/download/v0.7.10/panel.tar.gz | tar --strip-components=1 -xzv
    chmod -R 755 storage/* bootstrap/cache
    composer install --no-dev --optimize-autoloader
    ...
    chown -R www-data:www-data *
    output "Your panel has been updated to version 0.7.10."
    php artisan up
...
ssl_certs(){
    ...
    elif [ "$lsb_dist" =  "debian" ] && [ "$dist_version" = "9" ]; then
